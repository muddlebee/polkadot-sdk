...
static RISCV_ENGINE: OnceLock<polkavm::Engine> = OnceLock::new();

fn riscv_engine() -> &'static polkavm::Engine {
	RISCV_ENGINE.get_or_init(|| {
...
impl HostState {
	...
	pub fn new(allocator: FreeingBumpHeapAllocator) -> Self {
		HostState { allocator: Some(allocator), panic_message: None, riscv_data: None }
	}
...
impl<'a> sp_wasm_interface::Riscv for HostContext<'a> {
	fn execute(
		...
		let mut context = RiscvContext {
			host: self,
			data: RiscvData { syscall_handler, state_ptr, caller: None },
		};
	...
	fn read_memory(
		...
		let instance = self.riscv_data_mut().ok_or("No riscv instance found")?.caller_mut();
...
impl<'a, 'b> RiscvContext<'a, 'b> {
	fn shared_state_mut(&mut self) -> Option<&mut RiscvState<()>> {
		...
		let scoped =
			buf.get_mut(offset..offset.saturating_add(mem::size_of::<RiscvState<()>>()))?;
...
impl RiscvData {
	fn caller_mut(&mut self) -> &mut polkavm::Caller<'static, RiscvContext<'static, 'static>> {
		let caller = self
			.caller
			.expect("Is always set by ecall. This function can only be called during ecall; qed");
		let caller = unsafe { &mut *caller };
		caller
			.downcast_mut()
			.expect("We always put the correct type there in this file; qed")
	}
}

fn on_ecall(
	mut riscv_caller: polkavm::Caller<'_, RiscvContext<'_, '_>>,
	syscall_no: u32,
) -> Result<(), polkavm::Trap> {
	use polkavm::Reg;
	...
	let mut riscv_caller: polkavm::Caller<'static, RiscvContext<'static, 'static>> =
		unsafe { mem::transmute(riscv_caller) };
